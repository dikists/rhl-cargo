        ...
        <div class="col-xl-12 col-lg-12">
            ...
            <div class="card mb-5">
                <div class="card-header border d-flex flex-row border-primary align-items-center justify-content-between">
                    <h5 class="card-title mb-0">Edit Uang Jalan</h5>
                    <a href="<?= base_url('admin/uang_jalan'); ?>" class="btn btn-primary btn-sm"><i class="fas fa-arrow-left"></i> Kembali </a>
                </div>
                <div class="card-body border border-primary">
                    <form method="post" action="<?= base_url('admin/uang_jalan/update'); ?>">
                        <input type="hidden" name="id" value="<?= $uang_jalan['id']; ?>">
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                <label>Tanggal</label>
                                <input type="date" name="tanggal" class="form-control datepicker" autocomplete="off" value="<?= $uang_jalan['tanggal']; ?>" required>
                            </div>
                            <div class="form-group col-md-6">
                                <label>Driver</label>
                                <select name="user_id" id="user_id" class="form-control mySelect2" required>
                                    ...
                                    <?php foreach ($drivers as $data): ?>
                                        <?php $selected = ($data['id'] == $uang_jalan['user_id']) ? 'selected' : ''; ?>
                                        <option value="<?= $data['id']; ?>" <?= $selected; ?> data-username="<?= ucwords($data['username']); ?>"><?= ucwords($data['username']); ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            <div class="form-group col-md-6">
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                <label>Tujuan</label>
                                <input type="text" name="tujuan" class="form-control" required value="<?= $uang_jalan['tujuan']; ?>">
                            </div>
                            <div class="form-group col-md-6">
                                <label>Jumlah</label>
                                <input type="text" name="jumlah" class="form-control number" required value="<?= $uang_jalan['jumlah']; ?>">
                            </div>
                        </div>
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                <label>Keterangan</label>
                                <textarea name="keterangan" class="form-control"><?= $uang_jalan['keterangan']; ?></textarea>
                            </div>
                        </div>
